...
def logoff_student(idno):
    ...
    response = admin_account.log_off_student(idno=idno)
    if response['success']:
        return jsonify({"message": f"Student {idno} logged off successfully"}), 200
    else:
        return jsonify({"error": "Failed to update student status"}), 500
